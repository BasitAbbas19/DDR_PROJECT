...
app.get("/vendors/:id", (req, res) => {
  client.query(
    `Select * from vendor where user_id=${req.params.id}`,
    (err, result) => {
      if (!err) {
...
app.post("/vendor", (req, res) => {
  ...
  client.query(
    `select * from vendor_details  where email='${user.email}'`,
    (err, result) => {
    ...
    (err, result) => {
      if (result.rows.length >= 1) {
        ...
        if (user.password === result.rows[0].password) {
          client.query(
            `UPDATE vendor_details  SET online_status='${true}'  where email='${
              user.email
            }'`,
            (err, result) => {
...
app.post("/create_session", (req, res) => {
  ...
  client.query(
    `select * from session where sname='${user.sname}'`,
    (err, result) => {
      if (result.rows.length >= 1) {
        res.status(201);
        ...
        client.query(
          ...
          (err, result) => {
            if (result.rows.length >= 1) {
              ...
              client.query(
                `insert into client(email) values('${user.email}')`,
                (err, result) => {
            ...
            client.query(
              `insert into session(sname,client_email) values('${user.sname}','${user.email}')`,
              (err, result) => {
                if (!err) {
  ...
  if (!stack.isEmpty()) {
    ...
    for (let i = 0; i < x; i++) {
      ...
      if (dict[vendor_email] < 2) {
        ...
        sname = queue.dequeue();
        client.query(
          `Update session set vendor_email='${vendor_email}' where sname='${user.sname}'`,
          (err, result) => {
...
app.put("/get_client", (req, res) => {
  const user = req.body;
  client.query(
    `select * from session where vendor_email='${
      user.email
    }' AND waiting='${true}'`,
    (err, result) => {
      if (result.rows.length >= 1) {
        ...
        client.query(
          `Update session set waiting='${false}' where sname='${
            result.rows[0]["sname"]
          }'`,
          (err, result) => {}
        );
...
app.post("/rate_session", (req, res) => {
  const user = req.body;
  client.query(
    `insert into rating (sname,vendor_email,client_email,score) VALUES('${user.sname}','${user.vendor_email}','${user.client_email}',${user.score})`,
    (err, result) => {
      if (!err) {
